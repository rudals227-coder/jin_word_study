...
- `#/notes/<n>` → 그 레벨의 틀린 단어 노트(부모용, 자물쇠 해제 필요)
...
1. 안 본 단어를 **5개씩**(`INTRO_BATCH`) 소개 카드로 먼저 보여준다.
   뜻은 감춘 채 단어·예문·🔊 만 띄우고, **[뜻 보기]** 를 누르면 공개한 뒤
   `REVEAL_MS` 후 자동으로 다음 카드로 넘어간다(먼저 떠올려 보게 하려고).
2. 그 5개만 문제로 낸다(소개 안 한 단어는 안 나온다)
3. 틀리면 정답을 알려주지 않고 풀에 남긴다(레벨 1 과 동일). 대신 **틀린 기록이 쌓인다**
4. 덱의 단어를 전부 맞히면 완료 화면. 레벨 전체가 끝나면 '레벨 완료'
5. 홈의 **📕 틀린 단어** 는 자물쇠를 풀면 언제든 들어간다(`screens/notes/`).
   - **목록**: 틀린 단어를 한눈에. 많이 틀린 것부터 뜻·예문·🔊 와 함께.
   - **🔁 재도전**: 그 단어들로 4지선다를 다시 푼다. **맞힌 단어만 목록에서 빠진다**(`clearWrong`).
   자기신고 버튼으로 지우지 않는다 — 실제로 맞혀야 지워진다. 채점은 평소대로 진행상황에 기록된다.
